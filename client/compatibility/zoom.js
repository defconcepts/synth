...
function interpolate1(ux0, uy0, w0, w1) {
  var S = Math.abs(Math.log(w1 / w0)) / p,
      k = w1 < w0 ? -1 : 1;
...
function interpolate2(ux0, uy0, ux1, uy1, w0, w1) {
  var dx = ux1 - ux0,
      ...
      d2 = dx * dx + dy * dy,
      d1 = Math.sqrt(d2),
      b0 = (w1 * w1 - w0 * w0 + p4 * d2) / (2 * w0 * p2 * d1),
      b1 = (w1 * w1 - w0 * w0 - p4 * d2) / (2 * w1 * p2 * d1),
  ...
  function interpolate(t) {
    var s = t * S,
        u = w0 / (p2 * d1) * (cosh(r0) * tanh(p * s + r0) - sinh(r0));
    return [
      ux0 + u * dx,
      uy0 + u * dy,
      w0 * cosh(r0) / cosh(p * s + r0)
    ];
  };
